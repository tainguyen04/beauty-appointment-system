...
const readGuestMessages = () => {
  try {
    return (JSON.parse(sessionStorage.getItem(GUEST_MESSAGES_KEY)) ?? []).slice(
      -MAX_GUEST_MESSAGES,
    );
  } catch {
...
const AIChatPopup = () => {
  ...
  const saveGuestMessages = (nextMessages) => {
    if (!isLoggedIn)
      sessionStorage.setItem(GUEST_MESSAGES_KEY, JSON.stringify(nextMessages));
  };
  ...
  const handleSend = async () => {
    ...
    const pendingMessages = isLoggedIn
      ...
      : [...previousMessages, { role: "User", content }].slice(
          -MAX_GUEST_MESSAGES,
        );
    ...
    try {
      const response = await aiApi.chat({
        ...
        prompt: content,
        guestMessages: isLoggedIn
          ? []
          : previousMessages.slice(-MAX_GUEST_MESSAGES),
      });
      ...
      if (!isLoggedIn)
        completedMessages = completedMessages.slice(-MAX_GUEST_MESSAGES);
          ...
          <Card
            ...
            extra={
              <Flex gap={2}>
                ...
                {isLoggedIn && (
                  <Tooltip
                    title={showConversations ? "Ẩn lịch sử" : "Xem lịch sử"}
                  >
                    <Button
            ...
            styles={{
              header: { minHeight: 52, padding: "0 12px" },
              body: { padding: 0 },
            }}
          >
            <div
                ...
                >
                  {messages.length === 0 && !loading && !loadingHistory && (
                    <Empty
                      ...
                      description="Hãy đặt câu hỏi cho trợ lý"
                    />
                  )}
                  ...
                  <div style={{ maxHeight: 220, overflowY: "auto" }}>
                    ...
                    {conversations.length > 0 && (
                      <Menu
                        ...
                        onClick={({ key }) =>
                          handleSelectConversation(Number(key))
                        }
