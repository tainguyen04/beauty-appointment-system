...
const AppointmentManager = () => {
  const userFromStorage = GetUser(); // Lấy thông tin đã lưu ở Local/Session
  const isAdmin = userFromStorage?.role === 'Admin' ? true : false;
  ...
  const columns = useMemo(() => {
    const baseCols = [
      ...
      { title: 'Nhân viên', dataIndex: 'staffName', render: (name) => <Text strong>{name || 'Chưa phân công'}</Text> },
      { 
        title: 'Khu vực', 
        dataIndex: 'wardName', 
        width: 150,
  ...
  }, [isAdmin, handleEdit, handleUpdateStatus, handleDelete]);
